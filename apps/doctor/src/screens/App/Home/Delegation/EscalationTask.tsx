...
const ESCALATION_TO_OPTIONS = ["Provider", "Office Manager", "Another Staff role"];
const ASSIGN_TO_OPTIONS = ["Dr. Soliman", "Dr. Marcus", "Nurse Rebecca", "Office Manager"];
...
const EscalationTask = () => {
  ...
  const escalationToSheetRef = useRef<BSModal>(null);
  const assignToSheetRef = useRef<BSModal>(null);
  const prioritySheetRef = useRef<BSModal>(null);

  const showAssignTo = escalationTo.trim().length > 0;
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          />
          <Text style={styles.headerTitle}>Escalation Task</Text>
        ...
        >
          ...
          >
            <Text style={styles.label}>Escalation to</Text>
            <Pressable onPress={() => escalationToSheetRef.current?.present()}>
            ...
            {showAssignTo && (
              <>
                ...
                <Pressable onPress={() => assignToSheetRef.current?.present()}>
                  <View pointerEvents="none">
                    <InputField
                      value={assignTo}
                      placeholder="Select"
          ...
          <View style={styles.footer}>
            <View style={styles.footerButtons}>
              ...
              <ReusableButton
                title="Escalate Task"
                containerStyle={styles.escalateBtn}
        ...
        <SelectBottomSheetModal
          ref={escalationToSheetRef}
          title="Select Escalation to"
          options={ESCALATION_TO_OPTIONS}
          selectedValue={escalationTo}
          onSelectDone={(value) => {
            setEscalationTo(value);
            setAssignTo("");
          }}
        />
        <SelectBottomSheetModal
          ref={assignToSheetRef}
          title="Select Staff"
          options={ASSIGN_TO_OPTIONS}
          selectedValue={assignTo}
          onSelectDone={setAssignTo}
        />
        <SelectBottomSheetModal
...
const styles = StyleSheet.create({
  ...
  escalateBtn: { flex: 1, height: 48, borderRadius: 24 },
  escalateText: { color: "#FFFFFF", fontSize: 16, fontWeight: "500" },
