...
const SEND_BACK_OPTIONS = ["Same Staff", "Different Staff"];
const SEND_TO_OPTIONS = ["Maria (MA)", "Office Manager", "Billing", "Scheduler", "Other Provider"];
...
const RejectTask = () => {
  ...
  const [sendBackTo, setSendBackTo] = useState("Different Staff");
  const [sendTo, setSendTo] = useState("");
  const [notes, setNotes] = useState("");

  const sendBackBottomSheetRef = useRef<BSModal>(null);
  const sendToBottomSheetRef = useRef<BSModal>(null);

  const showSendTo = sendBackTo === "Different Staff";
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          />
          <Text style={styles.headerTitle}>Reject</Text>
        ...
        >
          ...
          >
            ...
            <Pressable onPress={() => sendBackBottomSheetRef.current?.present()}>
            ...
            {showSendTo && (
              <>
                ...
                <Pressable onPress={() => sendToBottomSheetRef.current?.present()}>
                  <View pointerEvents="none">
                    <InputField
                      value={sendTo}
                      placeholder="Select"
        ...
        <SelectBottomSheetModal
          ref={sendBackBottomSheetRef}
          title="Send Back To"
          options={SEND_BACK_OPTIONS}
          selectedValue={sendBackTo}
          onSelectDone={(value) => {
            setSendBackTo(value);
            if (value !== "Different Staff") setSendTo("");
          }}
        />
        <SelectBottomSheetModal
          ref={sendToBottomSheetRef}
          title="Send To"
          options={SEND_TO_OPTIONS}
          selectedValue={sendTo}
          onSelectDone={setSendTo}
        />
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  rejectText: { color: COLORS.ALERT, fontSize: 16, fontWeight: "500" },
